...
public class BatchAnalyticsDriver implements Serializable {

    public static final Logger LOGGER = Logger.getLogger(BatchAnalyticsDriver.class);
    ...
    static Map<Integer,JavaPairRDD<String,String>> pidRDDMap= new HashMap<>();
    static Map<JavaPairRDD<String,String>,Integer> rddPidMap= new HashMap<>();
    public Map<Integer, JavaPairRDD<String,WrapperMessage>> transformedRDDMap = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //sc.checkpoint("/tmp/checkpoint/"+parentProcessId);
            ...
            BatchAnalyticsDriver batchAnalyticsDriver = new BatchAnalyticsDriver();
            ...
            while (!currentUpstreamList.isEmpty()) {
                ...
                batchAnalyticsDriver.identifyFlows(currentUpstreamList, nextPidMap, parentProcessId);
            }
            batchAnalyticsDriver.createRDD(sc, listOfSourcePids);
            batchAnalyticsDriver.invokeRDDOperations(emptyRDD,sc, listOfSourcePids, prevMap, nextPidMap,broadcastMap);
    ...
    public void createRDD(JavaSparkContext sc,List<Integer> listOfSourcePids) throws Exception{
        try {
            System.out.println(" Inside create rdd");
        for (Integer pid : listOfSourcePids) {
            ...
            String sourceClassName = SOURCEPACKAGE+sourceType+"Source";
                ...
                JavaPairRDD<String,String> javaRDD = sourceObject.execute(sc, pid);
                pidRDDMap.put(pid,javaRDD);
                rddPidMap.put(javaRDD,pid);
            }
        }catch (Exception e){
            LOGGER.info(e);
            e.printStackTrace();
        ...
        try {
            ...
            for (Integer currentPid : currentUpstreamList) {
                ...
                for (int i = 0; i < nextPidArray.length; i++) {
                    ...
                    new BatchAnalyticsDriver().add(nextPids[i], currentPid, prevMapTemp);
                    new BatchAnalyticsDriver().add(nextPids[i], currentPid, prevMap);
                }
            }
    ...
    public JavaPairRDD<String,WrapperMessage> convertToRDDWrapperMessage(JavaPairRDD<String,String> rdd, int pid){
        JavaPairRDD<String,WrapperMessage> wrapperRDD= rdd.mapValues(v -> converter(v,pid));
        return wrapperRDD;
    }
    ...
    public void invokeRDDOperations(JavaRDD emptyRDD,JavaSparkContext sc, List<Integer> listOfSourcePids, Map<Integer, Set<Integer>> prevMap, Map<Integer, String> nextPidMap,Map<String,Broadcast<HashMap<String,String>>> broadcastMap) throws Exception {
        System.out.println(" inside invoke rdd");
            System.out.println("prevMap = " + prevMap);
            //iterate through each source and create respective dataFrames for sources
            for (Integer pid : pidRDDMap.keySet()) {
                System.out.println("pid = " + pid);
                System.out.println("FetchingRDD for source pid= " + pid);
                JavaPairRDD<String,String> msgDataLoad = pidRDDMap.get(pid);
                JavaPairRDD<String, WrapperMessage> wrapperRDD = null;
                GetProperties getProperties = new GetProperties();
                ...
                if(format.equalsIgnoreCase("Json")) {
                    /*SQLContext sqlContext = new SQLContext(sc);
                            ...
                            JavaRDD<String> javaRDD = msgDataLoad.map(s -> s._2);
                            javaRDD.take(15);
                            JavaRDD<Row> rowJavaRDD = sqlContext.read().schema(schema).json(javaRDD).javaRDD();
                            rowJavaRDD.take(15);
                            wrapperRDD = rowJavaRDD.mapToPair(row -> new Tuple2<String, WrapperMessage>(null,new WrapperMessage(row)));*/

                }
                /*else if(format.equalsIgnoreCase("XML")){
                    SQLContext sqlContext = new SQLContext(sc);
                    wrapperRDD = msgDataLoad.transformToPair(new Function<JavaPairRDD<String, String>, JavaPairRDD<String, WrapperMessage>>() {
                        @Override
                    ...
                    wrapperRDD = convertToRDDWrapperMessage(msgDataLoad, rddPidMap.get(msgDataLoad));
                }
                transformedRDDMap.put(pid,wrapperRDD);

                transformAndEmit(emptyRDD,nextPidMap.get(pid), transformedRDDMap,schema,broadcastMap,sc);
            }
    }



    public void transformAndEmit(JavaRDD emptyRDD, String nextPidString, Map<Integer, JavaPairRDD<String,WrapperMessage>> transformedRDDMap,StructType schema,Map<String,Broadcast<HashMap<String,String>>> broadcastMap,JavaSparkContext sc) throws Exception{
        try {

            System.out.println("nextPidString = " + nextPidString);
            System.out.println("transformedRDDMap = " + transformedRDDMap);
            if (!nextPidString.equals(nextPidMap.get(parentProcessId))) { //condition occurs when all emitters are finished and next is set to parentprocessid
                ...
                for (int i = 0; i < nextPidStringArray.length; i++) {
                    ...
                    if (nextPidInts[i].equals(parentProcessId)) {
                        ...
                        if (countEmitterCovered >= listOfEmitters.size()+ listOfPersistentStores.size()) {
                            System.out.println("clearing contents of transformedRDDMap before clearing= " + transformedRDDMap);
                            transformedRDDMap.clear();
                            System.out.println("clearing contents of transformedRDDMap before clearing= " + transformedRDDMap);
                ...
                for (int i = 0; i < nextPidInts.length; i++) {
                    for (Integer prevPid : prevMap.get(nextPidInts[i])) {
                        ...
                        else if (transformedRDDMap.get(prevPid) == null) {
                            return;
                        }
                    }
                }
                for (Integer pid : nextPidInts) {
                    ...
                    if (listOfTransformations.contains(pid)) {
                        ...
                        if(transformationType.equalsIgnoreCase("Custom")){
                            //JavaPairRDD<String, WrapperMessage> rddPostTransformation = Custom.transform(emptyRDD, transformedRDDMap, prevMap, pid, schema,broadcastMap,sc);
                            //transformedRDDMap.put(pid, rddPostTransformation);
                        }
                        else {
                            ...
                            JavaPairRDD<String, WrapperMessage> rddPostTransformation = transformationObject.transform(emptyRDD, transformedRDDMap, prevMap, pid, schema,broadcastMap,sc);
                    ...
                    /*if (listOfEmitters.contains(pid)) {
                        ...
                        for (Integer prevPid : prevPids) {
                            System.out.println("currently trying to emit rdd of prevPid = " + prevPid);
                            JavaPairRDD<String,WrapperMessage> prevRDD = transformedRDDMap.get(prevPid);
                            ...
                            String emitterClassName = EMITTERPACKAGE + emitterType + "Emitter";
                                ...
                                emitterObject.persist(prevRDD, pid, prevPid,schema);
                        }
                    }*/
                    ...
                    /*if (listOfMLModels.contains(pid)) {
                        ...
                        for (Integer prevPid : prevPids) {
                            ...
                            JavaPairRDD<String, WrapperMessage> rddPostTransformation = modelObject.transform(emptyRDD, transformedRDDMap, prevMap, pid, schema,broadcastMap,sc);
                    ...
                    if (listOfPersistentStores.contains(pid)) {
                        ...
                        Set<Integer> prevPids = prevMap.get(pid);
                        for (Integer prevPid : prevPids) {
                            System.out.println("currently trying to persist rdd of prevPid = " + prevPid);
                            JavaPairRDD<String,WrapperMessage> prevRDD = transformedRDDMap.get(prevPid);
                            ...
                            String persistentStoreClassName = PERSISTENTSTOREPACKAGE + persistentStoreType + "PersistentStore";
                                ...
                                persistentStoreObject.persist(emptyRDD,prevRDD, pid, prevPid, schema,broadcastMap,sc);
                        }
                    }
